    ...
    async execute(message, args, client) {
        ...
        if (arg1 === '-stop' || arg1 === '-parar' || arg1 === 'stop' || arg1 === 'parar') {
            if (message.author.id !== ownerId) {
                return message.reply('❌ Este comando de detención es exclusivo del propietario del bot.');
            }
        ...
        if (arg1 && arg1.length === 2) {
            ...
            if (session.error === 'NO_SUPPORTER') {
                return message.reply(`❌ No se puede iniciar el poblamiento de **${countryCode}**: No hay ningún token Supporter activo de ese país registrado en el pool del bot.`);
            }
            ...
            try {
                ...
                return message.reply(`📩 **¡Instrucciones enviadas a tu DM!** Revisa tus mensajes privados para copiar el comando de PowerShell para **${countryCode}**.`);
            } catch (err) {
                return message.reply(`⚠️ No pude enviarte el mensaje privado. Por favor activa los mensajes directos (DM) en el servidor para recibir tu clave de trabajador.`);
            }
        }
