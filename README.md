...
* **`s.top [usuario]`**
  ...
  * *Flags y Filtros:*
    ...
    * `-rev` (o `-reverse`, `-inv`): Invierte el orden del listado (ej: `-ar -rev` para menor AR primero, o `s.top -rev` para menor PP primero).
...
* **`s.snipes [usuario]`**
  ...
  * *Modos y Flags:*
    ...
    * `s.snipes -top`: Lista paginada interactiva de tus #1s nacionales.
      * *Filtros en `-top`:* `-sr>5 -sr<8` (rango de estrellas), `+<mods>`, `-? "<texto>"`, `-g <pp>`, `-r`, `-c`, `-acc`, `-bpm`, `-cs`, `-ar`, `-od`, `-hp`, `-rev`, `-i <n>`.
...
* **`s.nacional [código/nombre]`**
  ...
  * *Flags:*
    ...
    * `-pp`: Muestra el ranking de mejores jugadas por PP a nivel nacional (admite `-pais <código>`, `-m <mods>`, `-mx <mods>`, `-sr>X`, `-g <pp>`, `-? "<texto>"`, `-r`, `-c`, `-acc`, `-bpm`, `-cs`, `-ar`, `-od`, `-hp`, `-rev`, `-i <n>`).
